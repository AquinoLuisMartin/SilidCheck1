...
if (!isset($_SESSION['teacher_id'])) {
    echo '<tr><td colspan="7" style="text-align:center;padding:20px;">Not authorized.</td></tr>';
    exit();
}

$teacher_id = intval($_SESSION['teacher_id']);
$quiz_id = isset($_GET['quiz_id']) && !empty($_GET['quiz_id']) ? intval($_GET['quiz_id']) : 0;
$student_name = isset($_GET['student']) ? trim($_GET['student']) : '';
$date_filter = isset($_GET['date']) ? trim($_GET['date']) : '';

// Choose the appropriate stored procedure based on filters
if ($quiz_id > 0 && !empty($student_name) && !empty($date_filter)) {
    // All three filters
    $stmt = $conn->prepare("CALL FilterScoresByQuizStudentAndDate(?, ?, ?, ?)");
    $stmt->bind_param('iiss', $teacher_id, $quiz_id, $student_name, $date_filter);
} else if ($quiz_id > 0 && !empty($student_name)) {
